...
- 命中价格后，backend 返回对应商品行的购买坐标，引擎调用 `buy_item()` 执行购买交互。
- 点击购买后会先在购买弹窗区域确认同一商品图标出现，最多等待 5 秒，失败会点屏幕中心并重试，最多 3 次。
- 点击确认后会在顶部商品区域确认同一商品图标出现，最多等待 5 秒，成功后等待 1.5 秒再继续，失败同样最多重试 3 次。
- 刷新商店使用 `e7Helper` 参考坐标，刷新后等待首个商品行蓝色标记区域出现，超过 10 秒会终止本次运行。
- `buy_item()` 成功返回后，引擎才会累计购买数量和金币消耗。
...
- 识别参数说明：`docs/recognition_parameters.md`
